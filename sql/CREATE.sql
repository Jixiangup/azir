...
(
    ...
    verify      tinyint(1) default 1                 null comment '是否公开, 即为跳过权限验证, 即使没有权限也可以访问, 但是需要登录, 默认开启校验',
    )
    comment '路由菜单';
...
(
    ...
    parent_user_id bigint       default -1                                                null comment '所属父用户id, 如果为-1则表示为根用户',
    status         int          default 0                                                 null comment '用户当前状态，默认为0
0 正常
1 冻结
2 待激活'
    )
